...
namespace League\Configuration\Tests\Unit\Configuration;
...
final class ConfigurationTest extends TestCase
{
    public function testAddSchema(): void
    {
        ...
        $config->addSchema('structured', $structure);

        $this->assertSame(['foo' => 'bar'], $config->get('structured'));
    }

    public function testConfigurationProvider(): void
    {
        $config   = new Configuration();
        $provider = new class implements ConfigurationProviderInterface, ConfigurationAwareInterface {
        ...
        $this->assertSame($config, $provider->getConfiguration());
    }
    ...
    public function testInvalidConfigurationExceptionForConfigOption(): void
    {
        $t = InvalidConfigurationException::forConfigOption('foo', 'bar', 'baz');
        $this->assertInstanceOf(InvalidConfigurationException::class, $t);
        $this->assertSame('Invalid config option for "foo": bar (baz)', $t->getMessage());
        $this->assertNotInstanceOf(ValidationException::class, $t->getPrevious());
    }

    public function testInvalidConfigurationExceptionForParameter(): void
    {
        $t = InvalidConfigurationException::forParameter('foo', $this);
        $this->assertInstanceOf(InvalidConfigurationException::class, $t);
        $this->assertSame('Invalid foo: ' . self::class, $t->getMessage());
        $this->assertNotInstanceOf(ValidationException::class, $t->getPrevious());
    }
    ...
    public function testExists(): void
    {
        ...
        // Assert once more for code coverage on cache.
        $config->get('str');
        $this->assertTrue($config->exists('str'));
    }

    public function testImmutableData(): void
    {
        $config = new Configuration([
        ...
        // Retrieve the data and change it on the object returned.
        $data = $config->reader()->data();
        $data->set('foo', 'baz');

        // Ensure the config data wasn't actually changed.
        $this->assertSame('bar', $config->get('foo'));
    }
    ...
    public function testReader(): void
    {
        ...
        $this->assertTrue($config->exists('foo'));
        ...
        $this->assertFalse($config->exists('qux'));
        $this->assertFalse($reader->exists('qux'));
    }
}
